...
class ConvSTARCell(torch.nn.Module):
    ...
    def forward(
        self, inputs: torch.Tensor, prev_state: T.Union[None, torch.Tensor]
    ) -> torch.Tensor:
...
class ConvSTAR(torch.nn.Module):
    def __init__(
        ...
        """Generates a multi-layer convolutional GRU. Preserves spatial
        dimensions across cells, only altering depth.
    ...
    def forward(
        self, x: torch.Tensor, hidden: T.Union[None, T.List[torch.Tensor]]
    ) -> T.List[torch.Tensor]:
        """
...
class StarRNN(torch.nn.Module):
    ...
    def forward(
        self, x, hidden_s: T.Optional[torch.Tensor] = None
        ...
        if self.crop_type_layer:
            ...
            h = torch.cat([local_1, local_2], dim=1)
