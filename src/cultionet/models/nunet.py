...
class UNet2(nn.Module):
    ...
    def __init__(
        ...
        self.attention_0 = AttentionGate(
            high_channels=channels[3], low_channels=channels[4]
        )
        self.attention_1 = AttentionGate(
            high_channels=channels[2], low_channels=channels[3]
        )
        self.attention_2 = AttentionGate(
            high_channels=channels[1], low_channels=channels[2]
        )
        self.attention_3 = AttentionGate(
            high_channels=channels[0], low_channels=channels[1]
        )

        if boundary_layer:
            # Right stream
            self.bound4_1 = DoubleConv(channels[4] + channels[4], channels[0])
            self.bound3_1 = DoubleConv(
                channels[0] + channels[3] * 2, channels[0]
            )
            self.bound2_1 = DoubleConv(
                channels[0] + channels[2] * 2, channels[0]
            )
            self.bound1_1 = DoubleConv(
                channels[0] + channels[1] * 2, channels[0]
            )
            self.bound0_1 = DoubleConv(
                channels[0] + channels[0] * 2, channels[0]
            )
            # Left stream
    ...
    def forward(
        ...
        # 1/1
        x0_1 = self.conv0_1(
            torch.cat([x0_0, self.up(x1_0, size=x0_0.shape[-2:])], dim=1)
        )
        ...
        # 1/2
        x1_1 = self.conv1_1(
            torch.cat([x1_0, self.up(x2_0, size=x1_0.shape[-2:])], dim=1)
        )
        # 1/1
        x0_2 = self.conv0_2(
            torch.cat([x0_0, x0_1, self.up(x1_1, size=x0_1.shape[-2:])], dim=1)
        )
        ...
        # 1/4
        x2_1 = self.conv2_1(
            torch.cat([x2_0, self.up(x3_0, size=x2_0.shape[-2:])], dim=1)
        )
        # 1/2
        x1_2 = self.conv1_2(
            torch.cat([x1_0, x1_1, self.up(x2_1, size=x1_1.shape[-2:])], dim=1)
        )
        # 1/1
        x0_3 = self.conv0_3(
            torch.cat(
                [x0_0, x0_1, x0_2, self.up(x1_2, size=x0_2.shape[-2:])], dim=1
            )
        )
        ...
        # 1/8
        x3_1 = self.conv3_1(
            torch.cat([x3_0, self.up(x4_0, size=x3_0.shape[-2:])], dim=1)
        )
        x2_1 = self.attention_1(x2_1, x3_1)
        # 1/4
        x2_2 = self.conv2_2(
            torch.cat([x2_0, x2_1, self.up(x3_1, size=x2_1.shape[-2:])], dim=1)
        )
        x1_2 = self.attention_2(x1_2, x2_2)
        # 1/2
        x1_3 = self.conv1_3(
            torch.cat(
                [x1_0, x1_1, x1_2, self.up(x2_2, size=x1_2.shape[-2:])], dim=1
            )
        )
        ...
        x0_4 = self.conv0_4(
            torch.cat(
                [x0_0, x0_1, x0_2, x0_3, self.up(x1_3, size=x0_3.shape[-2:])],
                dim=1,
            )
        )
        ...
        if self.boundary_layer:
            ...
            b1_0 = self.bound1_0(
                torch.cat([x1_0, self.bound0_0_pool(b0_0)], dim=1)
            )
            b2_0 = self.bound2_0(
                torch.cat([x2_0, self.bound1_0_pool(b1_0)], dim=1)
            )
            b3_0 = self.bound3_0(
                torch.cat([x3_0, self.bound2_0_pool(b2_0)], dim=1)
            )
            b4_0 = self.bound4_0(
                torch.cat([x4_0, self.bound3_0_pool(b3_0)], dim=1)
            )
            # Right stream
            b4_1 = self.bound4_1(torch.cat([b4_0, x4_0], dim=1))
            b3_1 = self.bound3_1(
                torch.cat(
                    [x3_1, b3_0, self.up(b4_1, size=x3_1.shape[-2:])], dim=1
                )
            )
            b2_1 = self.bound2_1(
                torch.cat(
                    [x2_2, b2_0, self.up(b3_1, size=x2_2.shape[-2:])], dim=1
                )
            )
            b1_1 = self.bound1_1(
                torch.cat(
                    [x1_3, b1_0, self.up(b2_1, size=x1_3.shape[-2:])], dim=1
                )
            )
            boundary = self.bound0_1(
                torch.cat(
                    [x0_4, b0_0, self.up(b1_1, size=x0_4.shape[-2:])], dim=1
                )
            )
...
class UNet3(nn.Module):
    ...
    def __init__(
        ...
        self.conv0_0 = SingleConv(
            in_channels, channels[0], activation_type=activation_type
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_2_2 = self.convs_2_2(
            x0_0=x0_0, x1_0=x1_0, x2_0=x2_0, h3_1=out_3_1, x4_0=x4_0
        )
        # 1/2 connection
        out_1_3 = self.convs_1_3(
            x0_0=x0_0, x1_0=x1_0, h2_2=out_2_2, h3_1=out_3_1, x4_0=x4_0
        )
        # 1/1 connection
        out_0_4 = self.convs_0_4(
            x0_0=x0_0, h1_3=out_1_3, h2_2=out_2_2, h3_1=out_3_1, x4_0=x4_0
        )
