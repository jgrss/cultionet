...
def inverse_transform(x: torch.Tensor, data_values: NormValues) -> torch.Tensor:
    ...
    return data_values.std*x[:, :-1] + data_values.mean


def get_norm_values(
    dataset: T.Union[EdgeDataset, torch.utils.data.Dataset], batch_size: int, num_workers: int = 0
) -> NormValues:
    ...
    if not isinstance(dataset, EdgeDataset):
        ...
        with tqdm(total=int(len(dataset)/batch_size), desc='Calculating means') as pbar:
        ...
        with tqdm(total=int(len(dataset)/batch_size), desc='Calculating SSEs') as pbar:
            for x, y in data_loader:
        ...
        data_module = EdgeDataModule(train_ds=dataset, batch_size=batch_size, num_workers=num_workers)

        data_sums = torch.zeros(dataset[0].x.shape[1]-1, dtype=torch.float)
        sse = torch.zeros(dataset[0].x.shape[1]-1, dtype=torch.float)
        pix_count = 0.0

        with tqdm(total=int(len(dataset)/batch_size), desc='Calculating means') as pbar:
            for batch in data_module.train_dataloader():
                data_sums += batch.x[:, :-1].sum(dim=0)
        ...
        with tqdm(total=int(len(dataset)/batch_size), desc='Calculating SSEs') as pbar:
            ...
            for batch in data_module.train_dataloader():
                sse += ((batch.x[:, :-1] - add_dims(data_means)).pow(2)).sum(dim=0)
    ...
    norm_values = NormValues(mean=data_means,
                             std=data_stds,
                             max=data_maxs)
